...
export async function substituirArquivo(id: number, arquivo: File) {
  const form = new FormData();
  form.append("arquivo", arquivo);
  return api.put<DocumentoResponse>(`/api/documentos/${id}/arquivo`, { body: form });
}

export async function atualizarMetadados(id: number, metadata: DocumentoMetadataRequest) {
