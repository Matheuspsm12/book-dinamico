...
export function UserMenu() {
  ...
  return (
    <>
      ...
      {showSenha && <AlterarSenhaModal email={user.email} onClose={() => setShowSenha(false)} />}
    </>
  );
}

function AlterarSenhaModal({ email, onClose }: { email: string; onClose: () => void }) {
      ...
      {enviado ? (
        ...
        <div className="space-y-4">
          <div className="rounded-md border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
            Ao confirmar, geramos uma <strong>nova senha temporária</strong> e enviamos
            ...
            <p className="mt-3 text-xs text-zinc-500">
              Sua senha atual será invalidada imediatamente. Se não receber o e-mail em 5
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={enviar} disabled={submitting}>
              <KeyRound size={14} /> {submitting ? "Enviando…" : "Enviar nova senha"}
            </Button>
          </div>
        </div>
      )}
