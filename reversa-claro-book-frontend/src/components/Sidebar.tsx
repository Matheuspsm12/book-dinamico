...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-30 flex h-full flex-col justify-between border-r border-zinc-200 bg-[var(--sidebar-bg)] shadow-sm transition-all duration-200",
        open ? "w-64" : "w-20",
      )}
    >
      <button
        onClick={() => setOpen(!open)}
        className="absolute -right-3 top-20 z-10 grid h-6 w-6 place-items-center rounded-full bg-white shadow ring-1 ring-zinc-200"
        aria-label="toggle sidebar"
      >
        <ChevronRight size={14} className={cn("transition", open && "rotate-180")} />
      </button>

      <div>
        <div className="flex h-16 items-center gap-2 px-3 border-b border-zinc-200/70">
          <img src="/img/logo_symbol_c.svg" alt="Claro" className={cn("ml-1", open ? "max-w-10" : "max-w-8")} />
          {open && (
        ...
        <nav className="mt-4 px-2">
          {open && <p className="px-3 mb-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Navegação</p>}
