...
export async function listar(): Promise<DocumentoResponse[]> {
  const { data } = await api.get<DocumentoResponse[]>("/api/documentos");
  return data;
}

export async function buscar(id: number): Promise<DocumentoResponse> {
...
export async function substituirArquivo(id: number, arquivo: File): Promise<DocumentoResponse> {
  const form = new FormData();
  form.append("arquivo", arquivo);
  const { data } = await api.put<DocumentoResponse>(`/api/documentos/${id}/arquivo`, form);
  return data;
}
...
export async function atualizarMetadados(
  ...
  const { data } = await api.put<DocumentoResponse>(`/api/documentos/${id}`, metadata);
  return data;
}

export async function deletar(id: number): Promise<void> {
  await api.delete<void>(`/api/documentos/${id}`);
}
